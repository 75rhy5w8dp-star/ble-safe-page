...
function createMcpServer(req) {
  const mcp=new McpServer({name:"coco-svakom-relay",version:RELAY_VERSION},{instructions:"Control the user's own nearby COCO or SVAKOM toy through an authenticated Bluefy relay. COCO has suction and tongue-vibration levels 0-20. The local rhythm tool returns immediately while Bluefy continues the program. If the user says 停止, stop, 红灯, or red light, call toy_stop immediately and do not start or adjust anything else in that response."});
